<template>
  <div v-if="show" :class="embedded ? 'w-full' : 'fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50'">
    <div :class="embedded ? 'w-full' : 'bg-white dark:bg-gray-800 rounded-xl shadow-lg w-full max-w-2xl mx-4'">
      ...
      <div :class="embedded ? 'space-y-3' : 'p-4 space-y-3 max-h-[70vh] overflow-y-auto'">
        <div v-if="embedded" class="mb-4">
          <h3 class="text-lg font-semibold dark:text-gray-100">记忆数据</h3>
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">查看、纠正或删除当前角色积累的长期记忆。</p>
        </div>
